  ...
  return (
    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4 mb-6">
      {levels.map((level, index) => {
        const isLocked = index > 3; // example: lock after 4 levels
        return (
          <button
            key={level.id}
            onClick={() => onSelect(level.id)}
            disabled={isLocked}
            className={`p-4 shadow rounded-lg text-center transition flex flex-col items-center justify-center gap-1 
              ${isLocked 
          ...
          >
            ...
            {isLocked && <FaLock className="text-gray-500 mt-1" />}
          </button>
        );
      })}
    </div>
